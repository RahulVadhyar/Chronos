...
void Chronos::Editor::GUI::config() {
  ...
  if (ImGui::BeginMainMenuBar()) {
    ...
    ImGui::MenuItem("Post Processing", nullptr,
                    &internal.showPostProcessingWindow);
  ...
  if (internal.showSettingsWindow) {
    ImGui::Begin("Settings");
    {
      ...
      if (ImGui::BeginCombo(
              "MSAA",
              ("MSAA " + std::to_string(params->settings->msaaSamples)).c_str(),
              0)) {
...
void Chronos::Editor::GUI::init(Device *device, GLFWwindow *window, SwapChain *swapChain,
  ...
  init_info.QueueFamily =
      findQueueFamilies(device->physicalDevice, surface).graphicsFamily.value();
  ...
  VkCommandBuffer command_buffer =
      beginSingleTimeCommands(commandPool, device->device);
